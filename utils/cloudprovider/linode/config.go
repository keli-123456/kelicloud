...
func (a *Addition) Normalize() {
	...
	for _, token := range a.Tokens {
		...
		token.AccountCompany = strings.TrimSpace(token.AccountCompany)
		...
		if existingIndex, exists := seenTokens[token.Token]; exists {
			...
			if token.ProfileEmail != "" {
				merged.ProfileEmail = token.ProfileEmail
			}
			if token.AccountCompany != "" {
				merged.AccountCompany = token.AccountCompany
			}
...
func (t *TokenRecord) HasSavedInstancePassword(instanceID int) bool {
	if t == nil || instanceID <= 0 {
		return false
	}
	return t.findInstanceCredential(instanceID) != nil
}

func (t *TokenRecord) SavedInstancePasswordUpdatedAt(instanceID int) string {
...
func (t *TokenRecord) SaveInstancePassword(instanceID int, instanceLabel, passwordMode, rootPassword string, updatedAt time.Time) error {
	if t == nil || instanceID <= 0 {
	...
	if existing := t.findInstanceCredential(instanceID); existing != nil {
		*existing = record
	} else {
...
func (t *TokenRecord) RevealInstancePassword(instanceID int) (*InstancePasswordView, error) {
	credential := t.findInstanceCredential(instanceID)
	if credential == nil {
		return nil, ErrSavedRootPasswordNotFound
	}
...
func (t *TokenRecord) RemoveSavedInstancePassword(instanceID int) bool {
	if t == nil || instanceID <= 0 || len(t.InstanceCredentials) == 0 {
		return false
	}
...
func (t *TokenRecord) SyncInstanceCredentialLabel(instanceID int, instanceLabel string) bool {
	credential := t.findInstanceCredential(instanceID)
	if credential == nil {
		return false
	}
...
func (t *TokenRecord) PruneInstanceCredentials(validInstanceIDs map[int]struct{}) bool {
...
func (t *TokenRecord) SetCheckResult(checkedAt time.Time, profile *Profile, account *Account, err error) {
	if t == nil {
		return
	}
	...
	t.LastError = ""
	if profile != nil {
		t.ProfileUsername = strings.TrimSpace(profile.Username)
		t.ProfileEmail = strings.TrimSpace(profile.Email)
	}
	if account != nil {
...
func (t *TokenRecord) findInstanceCredential(instanceID int) *InstanceCredentialRecord {
	if t == nil || instanceID <= 0 {
		return nil
	}
