...
#### 方式 B：Docker Compose + 远程 MySQL

1. 准备环境变量文件：
   ```bash
   cp .env.example .env
   ```
2. 编辑 `.env`，填写你的远程 MySQL 连接信息：
   - `KOMARI_DB_HOST`
   - `KOMARI_DB_PORT`
   - `KOMARI_DB_NAME`
   - `KOMARI_DB_USER`
...
5. 查看初始账号输出：
   ...
   ```
6. 在浏览器中访问 `http://<your_server_ip>:25774`。

> [!NOTE]
> 仓库内置的 `docker-compose.yml` 会基于当前仓库本地构建镜像，并连接到你已有的 MySQL 服务。数据库备份请使用 `mysqldump` 或数据库服务商提供的快照能力；面板内的数据库下载备份目前仅支持 SQLite。
...
1. 构建前端静态文件：
   ...
   ```
   默认会使用仓库中 [`frontend-source.env`](../frontend-source.env) 固定的前端仓库和提交。
   如果你已经有本地 `komari-web` 检出，也可以这样复用：
   ```bash
