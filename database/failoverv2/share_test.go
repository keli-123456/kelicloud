...
func TestFailoverV2ShareIsUserScoped(t *testing.T) {
	db := openFailoverV2TestDB(t)
	if err := db.AutoMigrate(&models.FailoverV2Share{}); err != nil {
		t.Fatalf("failed to migrate share schema: %v", err)
	}

	shareA := &models.FailoverV2Share{
		UserID:     "user-a",
		ShareToken: "token-a",
		ServiceID:  1001,
	}
	shareB := &models.FailoverV2Share{
		UserID:     "user-b",
		ShareToken: "token-b",
		ServiceID:  1001,
	}
	...
	loadedA, err := getShareByServiceForUserWithDB(db, "user-a", 1001)
	if err != nil {
	...
	loadedB, err := getShareByServiceForUserWithDB(db, "user-b", 1001)
	if err != nil {
...
func TestRecordFailoverV2ShareSingleUseConsumesOnce(t *testing.T) {
	db := openFailoverV2TestDB(t)
	if err := db.AutoMigrate(&models.FailoverV2Share{}); err != nil {
		t.Fatalf("failed to migrate share schema: %v", err)
	}
